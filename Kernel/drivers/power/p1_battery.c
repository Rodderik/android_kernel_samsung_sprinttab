...
#include "p1_battery.h"
#include <linux/mfd/max8998.h>
#include <mach/regs-clock.h>
#include <asm/io.h>


#define POLLING_INTERVAL	400
...
extern void smb136_charging(int source);
...
{
	if(chg->cable_status==1 || chg->cable_status ==2)
		return 1;
	else
		return 0;
...
static int p1_charging_control(struct chg_data *chg)
{
	int ret;
...
static __devinit int max8998_charger_probe(struct platform_device *pdev)
{
	...
	s3c_bat_create_attrs(chg->psy_bat.dev);
